...
from copilot.evaluate import load_holdout_surrogate_rmse, load_model_and_predict
...
from copilot.geometry import render_mesh_preview, resolve_stl_path
...
def main() -> None:
    ...
    run_options = sorted(table["run_id"].unique().tolist())
    ...
    schema = FeatureSchema.load() if (SETTINGS.models_dir / "feature_schema.json").exists() else None if (SETTINGS.models_dir / "feature_schema.json").exists() else None
    holdout_rmse = load_holdout_surrogate_rmse()
    ...
    with tabs[0]:
        ...
        with geo_col:
            ...
            if stl_path:
                ...
                st.info("No local STL for this run.")
        ...
        def render_param_slider(param: dict[str, Any]) -> None:
            ...
            st.caption(f"Baseline {base_value:.4g} · Δ {delta_pct or 0:.2f}%")
