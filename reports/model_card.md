...
It is intended to help compare nearby DrivAerML-style geometry variants when editing tabular parameters relative to a baseline.
...
- Prediction uncertainty is approximate unless stronger calibration methods are added.

## Optional Explainability and OOD Checks

`src/copilot/explain.py` provides SHAP summary and local contribution helpers. `src/copilot/ood.py` stores a training feature profile and computes min/max plus z-distance warnings for selected baselines or edited designs.
